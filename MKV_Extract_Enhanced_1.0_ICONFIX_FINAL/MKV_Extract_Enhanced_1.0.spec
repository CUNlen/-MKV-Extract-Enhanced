...
# 1.0: 不再 collect_submodules("PySide6")。
# 旧版把整个 PySide6 子模块树打进 EXE，会显著增加包体与启动解压/初始化时间。
# 代码实际只使用 QtCore / QtGui / QtWidgets，由 PyInstaller 正常分析即可。
...
a = Analysis(
    ["MKV_Extract_Enhanced_1.0.py"],
    pathex=[],
    binaries=[],
    datas=[("MKV_Extract_Enhanced.ico", ".")],
...
exe = EXE(
    ...
    name="MKV Extract Enhanced 1.0 x64",
    debug=False,
